...
@section('title', 'Order Now | Pituwolu')
...
<div class="pt-6 md:pt-8 pb-24 md:pb-32 bg-surface min-h-screen relative overflow-hidden">
    ...
    <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 md:px-8 relative z-10">
        ...
        <div class="text-center mb-8 md:mb-16 lg:mb-24">
            ...
            <h1 class="text-2xl sm:text-3xl md:text-6xl font-bold font-headline text-on-surface">Pesan Sekarang</h1>
            <p class="text-on-surface-variant font-body mt-4 md:mt-6 max-w-2xl mx-auto text-sm md:text-lg leading-relaxed shadow-sm px-2">Isi formulir pengantaran/pengambilan, selesaikan pembayaran, dan pesanan Anda otomatis diproses.</p>
        </div>
        ...
        <div class="w-full max-w-3xl mx-auto" x-data="checkoutLogic()">
            ...
            <div class="bg-surface-container border border-surface-container-highest p-6 sm:p-8 md:p-12 rounded-[20px] sm:rounded-[32px] md:rounded-[40px] shadow-sm relative overflow-hidden">
                ...
                <h2 class="text-2xl sm:text-3xl lg:text-4xl font-bold font-headline mb-6 md:mb-8 border-b pb-4 md:pb-6 border-surface-variant text-on-surface">Detail Pesanan</h2>
                ...
                <form id="order-form" class="space-y-6 md:space-y-8 relative z-10 font-body">
                    @csrf
                    ...
                    <div class="space-y-3 md:space-y-4 pt-4 md:pt-6">
                        <h3 class="text-lg md:text-xl font-bold font-headline text-on-surface flex items-center gap-2 mb-4 md:mb-6"><span class="material-symbols-outlined text-secondary text-xl md:text-2xl">schedule</span> Waktu Pengambilan / Makan</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                            <div>
                                <label class="block text-xs md:text-sm font-bold text-on-surface mb-2 font-label">Tanggal</label>
                            ...
                            <div>
                                <label class="block text-xs md:text-sm font-bold text-on-surface mb-2 font-label">Jam</label>
                    ...
                    <div class="space-y-3 md:space-y-4 pt-4 md:pt-6">
                        ...
                        <div>
                            ...
                            <textarea name="notes" x-model="form.notes" id="input-notes" rows="3" class="w-full px-4 md:px-5 py-3 md:py-4 rounded-lg md:rounded-2xl bg-surface-container-lowest border border-outline-variant focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition text-on-surface text-sm md:text-base resize-none" placeholder="Berikan catatan, contoh: Takeaway, kurangi gula, dll..."></textarea>
                        </div>
                    </div>
        ...
        Alpine.data('checkoutLogic', () => ({
            ...
            async submitAndPay() {
                ...
                if (!name || !phone || !date || !time) {
                    document.getElementById('payment-error').textContent = 'Harap lengkapi semua field wajib (Nama, WA, Tanggal, Jam).';
                ...
                if (totalPrice <= 0 || cart.length === 0) {
                    document.getElementById('payment-error').textContent = 'Keranjang Anda kosong. Silakan pilih menu terlebih dahulu.';
                    document.getElementById('payment-error').classList.remove('hidden');
                    return;
                }
                ...
                try {
                    ...
                    window.snap.pay(data.snap_token, {
                        onSuccess: function(result) {
                            ...
                            localStorage.removeItem('pituwolu_cart');
